...
const DashboardRBAC: React.FC = () => {
  ...
  // Problem Actions
  const handleProblemDelete = async (problemId: string, problemTitle: string) => {
    if (!confirm(`Are you sure you want to delete "${problemTitle}"? This action will permanently remove it.`)) return;
    
    try {
      setActionLoading(`delete-problem-${problemId}`);
      const response = await axios.delete(`${backendUrl}/dashboard/problem/delete`, {
        data: { problemId },
  ...
  const handleProblemEdit = async (problemId: string) => {
    try {
      ...
      showError('Failed to load problem edit data: ' + error.response?.data?.message);
    } finally {
      setActionLoading('');
    }
  };

  const handleProblemApprove = async (problemId: string, problemTitle: string) => {
    try {
      setActionLoading(`approve-problem-${problemId}`);
      const response = await axios.put(`${backendUrl}/dashboard/problem/approve`, {
        problemId
      }, {
        headers: { "Content-Type": "application/json" },
        withCredentials: true,
  ...
  const handleProblemReject = async (problemId: string, problemTitle: string) => {
    const reason = prompt('Enter rejection reason (optional):');
    
    try {
      setActionLoading(`reject-problem-${problemId}`);
      const response = await axios.put(`${backendUrl}/dashboard/problem/reject`, {
        problemId,
        reason
      }, {
        headers: { "Content-Type": "application/json" },
  ...
  const handleUserEdit = async (targetUserId: string) => {
    try {
      ...
      const response = await axios.get(`${backendUrl}/dashboard/user/edit/${targetUserId}`, {
        headers: { "Content-Type": "application/json" },
        withCredentials: true,
      });
      ...
      showError('Failed to load user edit data: ' + error.response?.data?.message);
    } finally {
      setActionLoading('');
    }
  };

  const handleUserSetRole = async (targetUserId: string, newRole: string, userName: string) => {
    if (!confirm(`Change "${userName}" role to "${newRole}"?`)) return;
    
    try {
      setActionLoading(`setrole-user-${targetUserId}`);
      const response = await axios.put(`${backendUrl}/dashboard/user/setrole`, {
        targetUserId,
        newRole
      }, {
        headers: { "Content-Type": "application/json" },
  ...
  const handleUserPromote = async (targetUserId: string, userName: string) => {
    ...
    try {
      ...
      const response = await axios.put(`${backendUrl}/dashboard/user/promote`, {
        targetUserId
      }, {
        headers: { "Content-Type": "application/json" },
        withCredentials: true,
  ...
  const handleUserDemote = async (targetUserId: string, userName: string) => {
    ...
    try {
      ...
      const response = await axios.put(`${backendUrl}/dashboard/user/demote`, {
        targetUserId
      }, {
        headers: { "Content-Type": "application/json" },
        withCredentials: true,
      ...
      if (response.data.success) {
        const message = response.data.data.status === 'pending_approval' ? 
  ...
  // Request Actions
  const handleRequestApprove = async (requestId: string, requestType: string) => {
    if (!confirm(`Approve this ${requestType.replace('_', ' ')} request?`)) return;
    
    try {
      setActionLoading(`approve-request-${requestId}`);
      const response = await axios.put(`${backendUrl}/dashboard/request/approve`, {
        requestId
      }, {
        headers: { "Content-Type": "application/json" },
        withCredentials: true,
      ...
      if (response.data.success) {
        showSuccess(`${requestType.replace('_', ' ')} request approved and executed`);
        loadTabData(activeTab);
      }
    } catch (error: any) {
  ...
  const handleRequestReject = async (requestId: string, requestType: string) => {
    const rejectionReason = prompt('Enter rejection reason (optional):');
    
    try {
      setActionLoading(`reject-request-${requestId}`);
      const response = await axios.put(`${backendUrl}/dashboard/request/reject`, {
        requestId,
        rejectionReason
      }, {
        headers: { "Content-Type": "application/json" },
  ...
  const handleAuditRevert = async (auditId: string, action: string) => {
    if (!confirm(`Are you sure you want to revert the "${action.replace('_', ' ')}" action? This will undo the original action.`)) return;
    
    try {
      setActionLoading(`revert-audit-${auditId}`);
      const response = await axios.post(`${backendUrl}/dashboard/audit/revert`, {
        auditId
      }, {
        headers: { "Content-Type": "application/json" },
        withCredentials: true,
      ...
      if (response.data.success) {
        showSuccess(`Action "${action.replace('_', ' ')}" reverted successfully`);
        loadTabData(activeTab);
      }
    } catch (error: any) {
          ...
          {tabData.requests.map((request: PendingRequest) => (
            <div key={request._id} className={styles.card}>
              ...
              <div className={styles.cardContent}>
                ...
                <p><strong>Role:</strong> {request.requestedBy.role}</p>
                <p><strong>Date:</strong> {new Date(request.createdAt).toLocaleDateString()}</p>
                {request.reason && <p><strong>Reason:</strong> {request.reason}</p>}
                
                {request.reviewedBy && (
                  <p><strong>Reviewed by:</strong> {request.reviewedBy.firstname} {request.reviewedBy.lastname}</p>
                )}
              </div>
              ...
              {request.status === 'pending' && (
                <div className={styles.actionButtons}>
                  <button 
                    ...
                    onClick={() => handleRequestApprove(request._id, request.requestType)}
                    disabled={isActionLoading(`approve-request-${request._id}`)}
                  >
                    {isActionLoading(`approve-request-${request._id}`) ? 'Approving...' : 'Approve'}
                  </button>
                  <button 
                    className={`${styles.btn} ${styles.danger}`}
                    onClick={() => handleRequestReject(request._id, request.requestType)}
                    disabled={isActionLoading(`reject-request-${request._id}`)}
                  >
                    {isActionLoading(`reject-request-${request._id}`) ? 'Rejecting...' : 'Reject'}
                  </button>
                </div>
              )}
            </div>
    ...
    <div className={styles.tabContent}>
            ...
            <div key={problem._id} className={styles.card}>
              ...
              <div className={styles.cardContent}>
                <p><strong>Status:</strong> 
                  <span className={`${styles.status} ${problem.isApproved ? styles.approved : styles.pending}`}>
                    {problem.isApproved ? 'Approved' : 'Pending'}
                  </span>
                </p>
                <p><strong>Author:</strong> {problem.problemAuthorId?.firstname} {problem.problemAuthorId?.lastname}</p>
                <p><strong>Created:</strong> {new Date(problem.problemCreatedDate).toLocaleDateString()}</p>
                <p><strong>Last Modified:</strong> {new Date(problem.problemLastModifiedDate).toLocaleDateString()}</p>
              ...
              <div className={styles.actionButtons}>
                ...
                >
                  {isActionLoading(`edit-problem-${problem._id}`) ? 'Loading...' : 'Edit'}
                </button>
                ...
                {!problem.isApproved ? (
                  <>
                    <button 
                      className={`${styles.btn} ${styles.success}`}
                      onClick={() => handleProblemApprove(problem._id, problem.title)}
                      disabled={isActionLoading(`approve-problem-${problem._id}`)}
                    >
                      {isActionLoading(`approve-problem-${problem._id}`) ? 'Approving...' : 'Approve'}
                    </button>
                    <button 
                      className={`${styles.btn} ${styles.warning}`}
                      onClick={() => handleProblemReject(problem._id, problem.title)}
                      disabled={isActionLoading(`reject-problem-${problem._id}`)}
                    >
                      {isActionLoading(`reject-problem-${problem._id}`) ? 'Rejecting...' : 'Reject'}
                    </button>
                  </>
                ) : null}
                ...
                <button 
                  className={`${styles.btn} ${styles.danger}`}
                  onClick={() => handleProblemDelete(problem._id, problem.title)}
                  disabled={isActionLoading(`delete-problem-${problem._id}`)}
                >
                  {isActionLoading(`delete-problem-${problem._id}`) ? 'Deleting...' : 'Delete'}
                </button>
              </div>
            </div>
          ))}
      ...
      {tabData.pagination && (
        <div className={styles.paginationInfo}>
          <p>
            Page {tabData.pagination.currentPage} of {tabData.pagination.totalPages} 
            ({tabData.pagination.totalItems} total problems)
          </p>
        </div>
      )}
    </div>
    ...
    <div className={styles.tabContent}>
          ...
          {tabData.users.map((user: User) => (
            <div key={user._id} className={styles.card}>
              ...
              <div className={styles.cardContent}>
                ...
                <p><strong>Solved Problems:</strong> {user.solvedProblems?.length || 0}</p>
                <p><strong>Attempted Problems:</strong> {user.attemptedProblems?.length || 0}</p>
                <p><strong>Bookmarked:</strong> {user.bookmarkedProblems?.length || 0}</p>
              </div>
              ...
              <div className={styles.userRoleSection}>
                ...
                <select 
                  ...
                  value={user.role}
                  onChange={(e) => handleUserSetRole(user._id, e.target.value, `${user.firstname} ${user.lastname}`)}
              ...
              <div className={styles.actionButtons}>
                ...
                >
                  {isActionLoading(`edit-user-${user._id}`) ? 'Loading...' : 'Edit'}
                </button>
                
                <button 
                  className={`${styles.btn} ${styles.success}`}
                  onClick={() => handleUserPromote(user._id, `${user.firstname} ${user.lastname}`)}
                  disabled={isActionLoading(`promote-user-${user._id}`) || user.role === 'super_admin'}
                >
                  {isActionLoading(`promote-user-${user._id}`) ? 'Promoting...' : 'Promote'}
                </button>
                
                <button 
                  className={`${styles.btn} ${styles.warning}`}
                  onClick={() => handleUserDemote(user._id, `${user.firstname} ${user.lastname}`)}
                  disabled={isActionLoading(`demote-user-${user._id}`) || user.role === 'user'}
                >
                  {isActionLoading(`demote-user-${user._id}`) ? 'Demoting...' : 'Demote'}
                </button>
                
                <button 
                  className={`${styles.btn} ${styles.danger}`}
                  onClick={() => handleUserDelete(user._id, `${user.firstname} ${user.lastname}`)}
                  disabled={isActionLoading(`delete-user-${user._id}`)}
                >
                  {isActionLoading(`delete-user-${user._id}`) ? 'Deleting...' : 'Delete'}
                </button>
              </div>
            </div>
          ))}
      ...
      {tabData.pagination && (
        <div className={styles.paginationInfo}>
          <p>
            Page {tabData.pagination.currentPage} of {tabData.pagination.totalPages} 
            ({tabData.pagination.totalItems} total users)
          </p>
        </div>
      )}
    </div>
        ...
        <div className={styles.grid}>
          {tabData.logs.map((log: AuditLog) => (
            <div key={log._id} className={styles.card}>
              ...
              <div className={styles.cardContent}>
                <p><strong>Performed by:</strong> {log.performedBy?.firstname} {log.performedBy?.lastname}</p>
                <p><strong>Role:</strong> {log.performedBy?.role}</p>
                <p><strong>Target:</strong> {log.targetType}</p>
              ...
              {log.reversible && (
                <div className={styles.actionButtons}>
                  ...
                  >
                    {isActionLoading(`revert-audit-${log._id}`) ? 'Reverting...' : 'Revert Action'}
                  </button>
                </div>
              )}
            </div>
    ...
    <div className={styles.dashboard}>
      <header className={styles.dashboardHeader}>
        ...
        <div className={styles.userInfo}>
          <span>Welcome, {dashboardData.user.firstname} {dashboardData.user.lastname}</span>
          <span className={`${styles.roleBadge} ${styles[dashboardData.user.role]}`}>
