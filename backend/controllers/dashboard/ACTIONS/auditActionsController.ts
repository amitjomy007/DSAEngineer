  ...
  try {
    ...
    switch (auditLog.action) {
      case "user_role_changed":
        ...
        break;
      case "user_deleted": // ✅ ADD THIS CASE
        revertResult = await revertUserDeletion(auditLog);
        break;
      case "user_role_changed":
        revertResult = await revertUserRoleChange(auditLog);
        break;
...
// Add this helper function to auditActionsController.ts
async function revertUserDeletion(auditLog: any) {
  try {
    const targetUserId = auditLog.targetId;

    // Restore the user by removing deleted flags
    ...
    return {
      action: "user_deletion_reverted",
      targetUserId: targetUserId,
      restoredUser: auditLog.metadata.deletedUser,
      success: true,
    };
  } catch (error:any) {
    return {
      action: "user_deletion_revert_failed",
      error: error.message,
      success: false,
    };
  }
...
// Update the main revertAuditAction function to include this case
